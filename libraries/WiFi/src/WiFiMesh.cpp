/*
  WiFiMesh.cpp - Mesh network node
...
WiFiMesh::WiFiMesh(String chip_id, std::function<String(String)> handler)
: _server(SERVER_PORT)
{
	_chip_id = chip_id;
	_ssid = String( String( SSID_PREFIX ) + _chip_id );
 ...
 *
 */
bool WiFiMesh::waitForClient(WiFiClient curr_client, int max_wait)
{
	int wait = max_wait;
 ...
 *
 */
bool WiFiMesh::exchangeInfo(String message, WiFiClient curr_client)
{
...
void WiFiMesh::attemptScan(String message)
{
	/* Scan for APs */
	...
	for (int i = 0; i < n; ++i) {
		...
		String target_chip_id = (current_ssid.substring(index,_ssid_prefix.length()));

		/* Connect to any _suitable_ APs which contain _ssid_prefix */
		if (index >= 0 && (target_chip_id == _ssid_prefix)) {
			WiFi.mode(WIFI_STA);
			delay(100);
